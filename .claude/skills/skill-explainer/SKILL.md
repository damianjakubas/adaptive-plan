...
description: >
  Analyze a skill to understand its mechanics, design rationale, and how to
  build something similar. Reads a target skill's SKILL.md (and any references/)
  and produces a 7-section report: Problem & Purpose, Chain Position, Anatomy
  Walkthrough, Key Mechanics, Design Decisions, Adaptation Guide, Building
  Something Similar. Use when the user asks "how does skill X work", "explain
  skill X", "analyze this skill", "demystify /10x-plan", "skill-explainer",
  "how is this skill built", or provides a path to a SKILL.md file.
...
- **Read-only.** This skill never writes or edits any file. Do not call Write, Edit, or any mutating tool.
- **Do not invoke the analyzed skill.** Only read its source files — never trigger the skill being explained.
- **STOP after the report.** Deliver the 7-section report and stop. Do not chain into any other skill.
- **Do not invent structure.** If a section has nothing meaningful to say (e.g., no chain position for a standalone skill), skip it — do not pad with generic filler.
